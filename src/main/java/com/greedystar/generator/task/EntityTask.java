...
public class EntityTask extends AbstractTask {
	...
	public void run() throws IOException, TemplateException {
		...
		String remarks = null;
		if (Mode.ENTITY_MAIN.equals(mode)) {
			className = ConfigUtil.getConfiguration().getName().getEntity().replace(Constant.PLACEHOLDER,
	...
	public String entityMethods(AbstractInvoker invoker) {
		...
		tableInfos.forEach(ForEachUtil.withIndex((info, index) -> {
			...
			String setter = String.format("public void set%s (%s %s) { this.%s = %s; } \n\n",
					StringUtil.firstToUpperCase(info.getPropertyName()), info.getPropertyType(), info.getPropertyName(),
	...
	/**
	 * 为实体属性生成swagger注解 我们不建议在entity（do）中使用swagger注解，在dto和vo中使用swagger注解更为优雅
	 *
	 * @param sb   StringBuilder对象
	 * @param info 列属性
	 */
	public void generateSwaggerAnnotation(StringBuilder sb, ColumnInfo info) {
		...
		sb.append(String.format("@ApiModelProperty(value = \"%s\", dataType = \"%s\")", info.getRemarks(),
				info.getPropertyType()));
		sb.append("\n");
	}
		...
		if (ConfigUtil.getConfiguration().isMybatisPlusEnable()) {
			if (info.isPrimaryKey()) {
				if (ConfigUtil.getConfiguration().getIdStrategy() == null
					...
					sb.append(Constant.SPACE_4).append(
							String.format("@TableId(value = \"%s\", type = IdType.AUTO)\n", info.getColumnName()));
				} else if (ConfigUtil.getConfiguration().getIdStrategy() == IdStrategy.UUID) {
					sb.append(Constant.SPACE_4).append(String
							.format("@TableId(value = \"%s\", type = IdType.ASSIGN_UUID)\n", info.getColumnName()));
				}
			} else {
				sb.append(Constant.SPACE_4)
			...
			if (info.isPrimaryKey()) {
				if (ConfigUtil.getConfiguration().getIdStrategy() == null
					...
					sb.append(Constant.SPACE_4)
							.append("@GenericGenerator(name = \"uuidGenerator\", strategy = \"uuid\")\n");
				}
			}
