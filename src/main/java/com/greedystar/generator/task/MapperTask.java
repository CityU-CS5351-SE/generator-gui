...
public class MapperTask extends AbstractTask {
	...
	public void run() throws IOException, TemplateException {
		...
		mapperData.put("ClassName", ConfigUtil.getConfiguration().getName().getEntity().replace(Constant.PLACEHOLDER,
				invoker.getClassName()));
		mapperData.put("EntityName", StringUtil.firstToLowerCase(invoker.getClassName()));
		mapperData.put("DaoClassName",
		...
		if (ConfigUtil.getConfiguration().isMapperUnderSource()) {
			...
			// filePath = FileUtil.getSourcePath() +
			...
			filePath = ConfigUtil.getConfiguration().getProjectFolderPath() + "\\"
					+ StringUtil.package2Path(ConfigUtil.getConfiguration().getPath().getMapper());
		} else {
			// 默认情况下，将Mapper映射文件放在resources下
			// filePath = FileUtil.getResourcePath()
			// +
			// StringUtil.package2Path(ConfigUtil.getConfiguration().getPath().getMapper());
			// TODO getFilePathFromGUI
	...
	public String columnMap() {
		...
		if (invoker.getParentTableInfos() != null) {
			invoker.getParentTableInfos().forEach(ForEachUtil.withIndex((info, index) -> {
				...
				if (!StringUtil.isEmpty(invoker.getRelationalTableName())
						|| !StringUtil.isEmpty(invoker.getParentForeignKey())) {
					sb.append(String.format("`%s`.`%s` AS `%ss.%s`,\n", invoker.getParentTableName(),
							info.getColumnName(), StringUtil.firstToLowerCase(invoker.getParentClassName()),
							info.getColumnName()));
				} else {
					sb.append(
	...
	public String association() {
		...
		sb.append(String.format("<association property=\"%s\" javaType=\"%s.%s\">\n",
				...
				ConfigUtil.getConfiguration().getPackageName() + "."
						+ ConfigUtil.getConfiguration().getPath().getEntity(),
				invoker.getParentClassName()));
		invoker.getParentTableInfos().forEach(ForEachUtil.withIndex((info, index) -> {
			if (info.isPrimaryKey()) {
				sb.append(Constant.SPACE_12)
						.append(String.format("<id column=\"%s.%s\" property=\"%s\" />\n",
								StringUtil.firstToLowerCase(invoker.getParentClassName()), info.getColumnName(),
								info.getPropertyName()));
			} else {
				sb.append(Constant.SPACE_12)
						.append(String.format("<result column=\"%s.%s\" property=\"%s\" />\n",
								StringUtil.firstToLowerCase(invoker.getParentClassName()), info.getColumnName(),
								info.getPropertyName()));
			}
		}));
	...
	public String collection() {
		...
		invoker.getParentTableInfos().forEach(ForEachUtil.withIndex((info, index) -> {
			if (info.isPrimaryKey()) {
				sb.append(Constant.SPACE_12)
						.append(String.format("<id column=\"%ss.%s\" property=\"%s\" />\n",
								StringUtil.firstToLowerCase(invoker.getParentClassName()), info.getColumnName(),
								info.getPropertyName()));
			} else {
				sb.append(Constant.SPACE_12)
						.append(String.format("<result column=\"%ss.%s\" property=\"%s\" />\n",
								StringUtil.firstToLowerCase(invoker.getParentClassName()), info.getColumnName(),
								info.getPropertyName()));
			}
		}));
	...
	public String joins() {
		...
		if (!StringUtil.isEmpty(invoker.getRelationalTableName())) {
			...
			sb.append(String.format("LEFT JOIN `%s` ON `%s`.`%s` = `%s`.`%s`", invoker.getParentTableName(),
					invoker.getParentTableName(),
					getPrimaryKeyColumnInfo(invoker.getParentTableInfos()).getColumnName(),
			...
			sb.append(String.format("LEFT JOIN `%s` ON `%s`.%s = `%s`.`%s`", invoker.getParentTableName(),
					invoker.getParentTableName(),
					getPrimaryKeyColumnInfo(invoker.getParentTableInfos()).getColumnName(), invoker.getTableName(),
					invoker.getForeignKey()));
		}
		return sb.toString();
	}
